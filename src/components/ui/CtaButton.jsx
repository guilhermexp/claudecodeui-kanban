...
export default function CtaButton({ onClick, disabled = false, children, className = '', icon = true, variant = 'default' }) {
  const base = 'inline-flex items-center gap-1.5 transition-colors disabled:opacity-60 text-sm rounded-xl';
  const variants = {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`${base} ${pad} ${variants[variant] || variants.default} ${className}`}
    >
