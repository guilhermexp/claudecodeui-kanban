import {
  ...
  TaskDetailsContext,
} from '../../components/vibe-kanban/context/taskDetailsContext';
...
const useNormalizedConversation = ({
  ...
  const createSSEConnection = useCallback(
    (processId: string, projectId: string): AbortController => {
      ...
      const baseUrl = `/api/vibe-kanban/projects/${projectId}/execution-processes/${processId}/normalized-logs/stream`;
      const url =
